...
function Sidebar() {
  ...
  useEffect(() => {
    const storedData = localStorage.getItem("Gn");
    if (storedData) {
 ...
 /* const names = ["1#F19576WGWhatsapp Group"];
  ...
  let display = JSON.parse(interests);*/
  ...
  return (
    <div>
      ...
      <div className={styles.addbtn} onClick={() => popupbox()}><div className={styles.btn}>+</div></div>
      {condition ?
          ...
          overflowY: 'scroll', outline: 'none'}}>
           ...
           {display.map((display, index) => (
            ...
            <div style={{ height: '10vh' }}>
              <span className={styles.user} style={{ background: display.substring(1, 8), borderRadius: '50%'}} 
              /*onClick={()=>{sethello('hello')}}*/>
                {display.substring(8, 10)}
              </span>&emsp;
              <span className={styles.user} style={{ color: 'black', fontSize: '25px' }}
               onClick={()=>opennotes(display.substring(0,1),display.substring(1,8),display.substring(8,10),
        ...
        :
        <div className={styles.scrollbar} style={{
          position: 'absolute', top: '15vh', left: '3vw', width: '20vw', height: '70vh',
          overflowX: 'hidden', overflowY: 'scroll', outline: 'none'
        }}>

          {display.map((display) => (
            <div style={{ height: '10vh' }}>
              <span className={styles.user} style={{background: display.substring(1, 8),
                borderRadius: '50%'
              }} /*onClick={()=>{sethello('hello')}}*/ >
                {display.substring(8, 10)}
              </span>&emsp;
              ...
              onClick={()=>opennotes(display.substring(0,1),display.substring(1,8),display.substring(8,10),
                display.substring(10,display.length))}> 
              {display.substring(10, display.length)}</span>
        ...
        :
        <div style={{position:'absolute' , left:'28vw', width:'72vw' ,height:'100vh' , background:'#DAE5F5'}}>
          <div style={{position:'absolute' , bottom:'4vh' , width:'71.8vw' , height:'30vh' , background:'#001F8B'
        , borderStyle:'solid' , borderWidth:'1px' , borderRadius:'9px' , borderColor:'#001F8B'}}>
           <textarea style={{position:'absolute' , top:'2vh' , left:'2vw' , width:'68vw' , height:'25vh' ,
        ...
        <div style={{position:'absolute',left:'30vw',width:'5vh',height:'5vh',background:bgcolor,top:'1.1vh',
        borderRadius:'50%'}}>
        <div style={{position:'absolute',color:'white',top:'1.2vh',left:'0.2vw'}} className={styles.initial}>{initials}</div>
        </div>
        <div style={{position:'absolute',left:'30vw',top:'9vh',width:'70vw',height:'56vh', 
        overflowX: 'hidden', overflowY: 'scroll', outline: 'none'}}>
         {con.map((con)=>(
            <>
            ...
            &emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;
            &emsp;&emsp;&emsp;&emsp;
            <span className={styles.dateandtime}>{con.substring(con.length-20,con.length-9)}</span>&emsp;&emsp;
      ...
      <Acceptinput trigger={buttonpopup} settrigger={setbuttonpopup} show={condition} setshow={setcondition} />
